...
class AutoRule(BaseTorchClassModel):
    ...
    def fit(self,
        ...
        if pretrained_model is not None:
            ...
            history_pretrain = {}
            last_step_log = {}
            with trange(n_steps, desc="[TRAIN] AutoRule pretrain student", unit="steps", disable=not verbose, ncols=150, position=0, leave=True) as pbar:
                cnt = 0
                step = 0
                model.train()
                optimizer.zero_grad()
        ...
        for i in range(n_iter):
            ...
            with trange(int(n_steps * self.unsup_prop), desc=f"[TRAIN@{i}] AutoRule-teacher-unsup", unit="steps", disable=not verbose, ncols=150, position=0, leave=True) as pbar:
                cnt = 0
                step = 0
                model.train()
                optimizer.zero_grad()
            ...
            with trange(n_steps, desc=f"[FINETUNE@{i}] AutoRule-teacher-sup", unit="steps", disable=not verbose, ncols=150, position=0, leave=True) as pbar:
                cnt = 0
                step = 0
                model.train()
                optimizer.zero_grad()
            ...
            with trange(int(n_steps * self.unsup_prop), desc=f"[TRAIN@{i}] AutoRule-student-unsup", unit="steps", disable=not verbose, ncols=150, position=0, leave=True) as pbar:
                cnt = 0
                step = 0
                model.train()
                optimizer.zero_grad()
            ...
            with trange(n_steps, desc=f"[FINETUNE@{i}] AutoRule-student-sup", unit="steps", disable=not verbose, ncols=150, position=0, leave=True) as pbar:
                cnt = 0
                step = 0
                model.train()
                optimizer.zero_grad()
    ...
    def predict_proba(self, dataset: Union[BaseDataset, DataLoader], mode: Optional[str] = 'student',
        ...
        if write_attns:
            ...
            out_fp = os.path.join(os.getenv('AMLT_OUTPUT_DIR', attns_prefix), 'attns.tsv')
